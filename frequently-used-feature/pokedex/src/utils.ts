export const mapColorToHex = (color?: string) => { // 포켓몬의 컬러를 문자열로 받아 hexCode 로 변환
  switch (color) {
    case 'white':
    case 'gray':
      return '#6B7280';
...
export const mapTypeToHex = (type?: string) => { // 포켓몬의 타입를 받아 hexCode(색) 로 변환
  switch (type) {
...
export const formatNumbering = (pokemonIndex: number | string): string => // 포켓몬의 인덱스를 받아서 #001 형태로 변환
