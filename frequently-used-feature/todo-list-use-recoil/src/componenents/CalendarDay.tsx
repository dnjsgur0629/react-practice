...
const CalendarDay: React.FC<Props> = ({ date }) => {  //prop으로 해당 날짜 받음
  ...
  const handleTodoFormModalOpen = (d: number) => {  // 할 일 등록 모달을 염
    // @ts-ignore
    setSelectedDate(new Date(selectedDate.setDate(d)));
    setTodoFormModalOpen(true);
  };

  const handleDateSelect = (d: number) => { // 날짜를 선택
    // @ts-ignore
    setSelectedDate(new Date(selectedDate.setDate(d)));
  }

  const handleTodoStatisticsModalOpen = (event: React.SyntheticEvent<HTMLDivElement>) => {  //통계 모달을 염
